...
def preprocess_addlabels(inFile, outFile,col, labels):
    command = "java  -cp " + WEKA_CLASSPATH + \
              " weka.filters.unsupervised.attribute.AddValues -C " + col + " -L " + labels  +\
              " -i " + inFile + \
              " -o " + outFile
    print(command)
    os.system(command)
...
def Randomforest_load_and_test(modelfile, train_file, testFile):
    command = "java -cp " + WEKA_CLASSPATH + \
                  " weka.classifiers.misc.InputMappedClassifier -I -M -trim " + \
                  " -L " + modelfile + \
                  " -t " + train_file + \
                  " -T " + testFile + \
                " -W weka.classifiers.trees.RandomForest -- -P 100 -I 100 -num-slots 1 -K 0 -M 1.0 -V 0.001 -S 1"
